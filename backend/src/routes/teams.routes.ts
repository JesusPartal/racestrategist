...
router.put('/:id/drivers/:driverId', (req: Request, res: Response) => {
  ...
  db.prepare(`UPDATE team_drivers SET name = COALESCE(?, name), accent_color = COALESCE(?, accent_color),
    license_class = COALESCE(?, license_class), i_rating = COALESCE(?, i_rating),
