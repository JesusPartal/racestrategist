...
function rowToToken(row: any): AgentToken {
  return {
    ...
    strategyDriverId: row.strategy_driver_id ?? null,
  };
}
...
export function ensureTable(): void {
  ...
  // Migrate: add strategy_driver_id if missing (v3 upgrade)
  try { db.exec("ALTER TABLE agent_tokens ADD COLUMN strategy_driver_id TEXT"); } catch {}
}
...
export function createAgentToken(teamId: string, driverId: string, driverName: string, createdBy: string, strategyDriverId: string | null = null): AgentToken {
  ...
  db.prepare(`INSERT INTO agent_tokens (id, team_id, driver_id, driver_name, token, created_at, expires_at, created_by, strategy_driver_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, teamId, driverId, driverName, token, Date.now(), expiresAt, createdBy, strategyDriverId);
...
export function validateAgentToken(token: string): { driverId: string; teamId: string; strategyDriverId: string | null } | null {
  const row = db.prepare('SELECT driver_id, team_id, expires_at, strategy_driver_id FROM agent_tokens WHERE token = ?').get(token) as any;
  if (!row) return null;
  // Check expiration
  ...
  return { driverId: row.driver_id, teamId: row.team_id, strategyDriverId: row.strategy_driver_id ?? null };
}
