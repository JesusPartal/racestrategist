...
export class StrategyApiService {
  ...
  async createStrategy(name: string, eventDurationMinutes: number, vehicleName: string, avgLapTimeMs: number, fuelPerLap: number, eventStartTime?: number, tankCapacity?: number): Promise<RaceStrategy> {
    return lastValueFrom(
      this.http.post<RaceStrategy>(`${API_BASE}/strategies`, { name, eventDurationMinutes, vehicleName, avgLapTimeMs, fuelPerLap, eventStartTime, tankCapacity })
    );
  }
